...
public class RiskLevelRulesService {
	...
	public RiskLevelEnum evaluateDiabetesRiskLevelForWomen (int patientAge,List<String> patientHistoryContent) {
		...
		riskLevelResultsList.add(evaluateInDangerRiskLevelForAnyPatientAboveCriticalAge(patientHistoryContent, patientAge));
		riskLevelResultsList.add(evaluateInDangerRiskLevelForWomenUnderCriticalAge(patientHistoryContent, patientAge));
		riskLevelResultsList.add(evaluateEarlyOnsetRiskLevelForAnyPatientAboveCriticalAge(patientHistoryContent, patientAge));
		riskLevelResultsList.add(evaluateEarlyOnsetRiskLevelForWomenUnderCriticalAge(patientHistoryContent, patientAge));

		RiskLevelEnum result = riskLevelResultsList.stream()
				.filter(r-> r != null)
				.findFirst().get();
	...
	private RiskLevelEnum evaluateNoneRiskLevel(List<String> patientHistoryContent) {
		...
		if (countMatchingKeywords(patientHistoryContent) == 1)
			result = RiskLevelEnum.NONE;
		return result;
	}

	private RiskLevelEnum evaluateBorderLineRiskLevel(List<String> patientHistoryContent,
			int patientAge) {
		RiskLevelEnum result = null;
	...
	private RiskLevelEnum evaluateInDangerRiskLevelForAnyPatientAboveCriticalAge(List<String> patientHistoryContent,int patientAge) {
		RiskLevelEnum result = null;
	...
	private RiskLevelEnum evaluateInDangerRiskLevelForWomenUnderCriticalAge(List<String> patientHistoryContent,int patientAge) {
		RiskLevelEnum result = null;
	...
	private RiskLevelEnum evaluateInDangerRiskLevelForMenUnderCriticalAge(List<String> patientHistoryContent,int patientAge) {
		RiskLevelEnum result = null;
	...
	private RiskLevelEnum evaluateEarlyOnsetRiskLevelForAnyPatientAboveCriticalAge(List<String> patientHistoryContent,int patientAge) {
		RiskLevelEnum result = null;
	...
	private RiskLevelEnum evaluateEarlyOnsetRiskLevelForWomenUnderCriticalAge(List<String> patientHistoryContent,int patientAge) {
		RiskLevelEnum result = null;
		if (!evaluateCriticalAge(patientAge) && countMatchingKeywords(patientHistoryContent) == 8)
			result = RiskLevelEnum.EARLY_ONSET;
		return result;
	...
	private RiskLevelEnum evaluateEarlyOnsetRiskLevelForMenUnderCriticalAge(List<String> patientHistoryContent,int patientAge) {
		RiskLevelEnum result = null;
	...
	private int countMatchingKeywords(List<String> patientHistoryContent) {
		...
		List<String> matchingKeywordsList = matchingKeywordsListWithDuplicates.stream()
				.distinct()
	...
	private List<String> matchingKeywordsListInString (String noteContent,List<String> keywordsList) {
		List<String> matchingKeyWordsList = keywordsList.stream()
				.filter(kw->noteContent.contains(kw)==true)
				.distinct()
				.collect(Collectors.toList());
